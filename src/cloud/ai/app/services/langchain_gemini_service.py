...
os.environ["OPENAI_API_KEY"] = "sk-proj-rXai4BV3vyOlaOMr3MLET3BlbkFJIKzY54tMRK5Bjm092QaK" # Get it at https://platform.openai.com/account/api-keys
...
@tool()
def get_train_travels() -> str:
    """Get the list of train travels."""
    results_api = "https://dadesobertes.fgc.cat/api/explore/v2.1/catalog/datasets/viajes-de-hoy/records?limit=20"

    response = requests.get(results_api)

    if response.status_code == 200:
        return response.json()
    
    return "Error getting train travels"
...
class LangChainService:
    def __init__(self, on_human_input=None, cli_mode=False):
        ...
        tools = load_tools(["human"], llm=llm, input_func=on_human_input)
        ...
        tools.append(get_train_travels)
